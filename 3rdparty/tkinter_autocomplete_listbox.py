"""
modified 2016-03-10 by Darren Ringer (dwringer@gmail.com)
based on version by
uroshekic
...
Changes:
    ...
    --------------------------
    - Allowed inclusion in the child of a toplevel inside a withdrawn Tk
    - Fixed click-to-select
"""
...
class AutocompleteEntry(Entry):
    def __init__(self, autocompleteList, *args, **kwargs):
        ...
        if 'matchesFunction' in kwargs:
            ...
            def matches(fieldValue, acListEntry):
                ...
                return (re.match(pattern, acListEntry) or
                        (SequenceMatcher(a=fieldValue.upper(),
                                         b=acListEntry.upper()).ratio() > 0.68) and
                        (len(fieldValue) > 3) )
        ...
        #self.var = self["textvariable"]  # self->kwargs
        #if self.var == '':
        #    self.var = self["textvariable"] = StringVar(args[0])  # self->kwargs
        ...
        if self.var.get() == '':
            ...
            if words:
                if not self.listboxUp:
                    self.listbox = Listbox(self.parent, width=self["width"], height=self.listboxLength)
                    self.listbox.bind("<ButtonRelease-1>", self.selection)  # <<ListboxSelect>>
    ...
    def selection(self, event):
        if self.listboxUp:
            ...
            self.var.set(val)  # self.listbox.get(ACTIVE))
            self.listbox.destroy()
            self.listboxUp = False
            self.icursor(END)
