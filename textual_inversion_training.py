...
class TextualInversionDataset(Dataset):
    def __init__(
        ...
        for file_path in self.data_root.iterdir():
            if file_path.is_file():
                if file_path.name.endswith(('.png','.PNG','.jpg','.JPG','.jpeg','.JPEG','.gif','.GIF')):
                    self.image_paths.append(self.data_root / file_path)
                elif file_path.name.endswith(('.txt','.TXT','.json','.JSON')):
                    with open(self.data_root / file_path, 'r') as json_file_in:
                        for k,v in json.load(json_file_in).items():
                            self.template_extras[k] = ((self.template_extras[k] + v)
                                                       if k in self.template_extras
                                                       else v)
        ...
        # {"*": ["exhaustive prompt examples of {}", ...]}
        # {"+": ["additional prompt examples of {}", ...]}
        # {"imagA.png": ["specific prompt examples of {} in specific image", ...]}

        if "*" in self.template_extras:
            self.templates = self.template_extras["*"]
        else:
            self.templates = (
                imagenet_style_templates_small
        ...
        if "+" in self.template_extras:
            self.templates += self.template_extras["+"]
        self.templates_by_image = {}

        # Precompute full list of templates for each image so they can be retrieved more efficiently
        for image_path in self.image_paths:
            self.templates_by_image[image_path.name] = self.templates + (self.template_extras[image_path.name]
                                                                        if image_path.name in self.template_extras
                                                                        else [])
    ...
    def __getitem__(self, i):
        ...
        text = random.choice(self.templates_by_image[image_path.name]).format(placeholder_string)
...
def do_textual_inversion_training(
    ...
    **kwargs,
    ...
    logger.info(f"  Prompts by image {train_dataset.templates_by_image}")
